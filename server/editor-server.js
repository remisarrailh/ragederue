...
const PROPDEFS_PATH = path.join(ROOT, 'js', 'config', 'propDefs.js');
...
function generateOneLevelJs(level) {
  ...
  // Unified objects list (props + containers merged)
  const objects = level.objects ?? [];
  lines.push('  objects: [');
  for (const obj of objects) {
    lines.push(`    { type: '${obj.type}', x: ${obj.x}, y: ${obj.y} },`);
  }
  lines.push('  ],');
...
function readPropDefs() {
  try {
    const src = fs.readFileSync(PROPDEFS_PATH, 'utf8');
    const cleaned = src
      .replace(/^export\s+const\s+/gm, 'var ')
      .replace(/^export\s+function\s+/gm, 'function ');
    const ctx = {};
    vm.runInNewContext(cleaned, ctx);
    return ctx.PROP_DEFS ?? {};
  } catch (e) {
    console.error('[EditorServer] readPropDefs error:', e.message);
    return {};
  }
}

function generatePropDefsJs(defs) {
  const lines = [];
  const L = (s = '') => lines.push(s);

  L('/**');
  L(' * propDefs.js — Définitions par texture pour tous les objets du monde.');
  L(' *');
  L(' * Chaque texture a UNE définition partagée par toutes ses instances.');
  L(' * Plus de scale, blocksPlayer, collision, etc. par instance.');
  L(' * Tout est centralisé ici.');
  L(' *');
  L(' * AUTO-GENERATED by editor server — NE PAS MODIFIER MANUELLEMENT');
  L(' * (sauf pour le seed initial)');
  L(' */');
  L();
  L('export const PROP_DEFS = {');
  for (const [key, def] of Object.entries(defs)) {
    const col = def.collision
      ? `{ width: ${def.collision.width}, height: ${def.collision.height}, offsetX: ${def.collision.offsetX ?? 0}, offsetY: ${def.collision.offsetY ?? 0} }`
      : 'null';
    const special = def.specialType ? `'${def.specialType}'` : 'null';
    L(`  '${key}': {`);
    L(`    scale: ${def.scale ?? 1},`);
    L(`    isContainer: ${!!def.isContainer},`);
    L(`    blocksPlayer: ${!!def.blocksPlayer},`);
    L(`    collision: ${col},`);
    L(`    specialType: ${special},`);
    L(`  },`);
  }
  L('};');
  L();
  L('/**');
  L(' * Retourne la définition d\'un type de prop.');
  L(' * Fallback safe si le type n\'existe pas.');
  L(' */');
  L('export function getPropDef(type) {');
  L('  return PROP_DEFS[type] ?? {');
  L('    scale: 1,');
  L('    isContainer: false,');
  L('    blocksPlayer: false,');
  L('    collision: null,');
  L('    specialType: null,');
  L('  };');
  L('}');
...
function savePropDefs(defs) {
  fs.writeFileSync(PROPDEFS_PATH, generatePropDefsJs(defs), 'utf8');
  console.log(`[EditorServer] propDefs.js saved (${Object.keys(defs).length} textures).`);
}
...
const server = http.createServer((req, res) => {
  ...
  // ── GET /propdefs ──────────────────────────────────────────────────
  if (req.method === 'GET' && pathname === '/propdefs') {
    try {
      json(res, 200, readPropDefs());
    } catch (e) {
      console.error('[EditorServer] readPropDefs error:', e.message);
      json(res, 500, { error: e.message });
    }
  ...
  // ── POST /propdefs ─────────────────────────────────────────────────
  if (req.method === 'POST' && pathname === '/propdefs') {
    let body = '';
    req.on('data', c => { body += c; });
    req.on('end', () => {
      try {
        const defs = JSON.parse(body);
        if (typeof defs !== 'object' || Array.isArray(defs)) throw new Error('payload must be an object');
        savePropDefs(defs);
        json(res, 200, { ok: true, count: Object.keys(defs).length });
      } catch (e) {
        console.error('[EditorServer] savePropDefs error:', e.message);
        json(res, 400, { error: e.message });
      }
    });
    return;
